                ...
                <div className="p-4">
                  <div className="flex justify-between items-center border-b-2 border-slate-900 pb-3 mb-3">
                    <div className="flex items-center gap-3">
                      <div className="bg-slate-950 p-1.5 rounded-xl border border-sky-400/40"><img src="/rtx-logo.png" alt="RTX Logo" className="h-10 w-auto" /></div>
                      <h2 className="text-lg font-black text-slate-900 dark:text-white">RTX للتجارة والتصنيع</h2>
                    </div>
                    <div className="text-left">
                      <div className="inline-block bg-gradient-to-r from-slate-900 to-[#0284c7] text-white px-4 py-1.5 rounded-lg text-xs font-black">{isAll ? `كشف مجمع (${getTypeLabel()}ين)` : `كشف حساب ${getTypeLabel()}`}</div>
                      <p className="text-xs text-slate-500 font-bold mt-1">تاريخ الاستخراج: {currentDateFormatted}</p>
                    </div>
                  </div>
                  <div className="mb-3 bg-sky-50/50 p-3 rounded-lg"><span className="text-xs font-bold text-slate-500">اسم الحساب: </span><span className="text-base font-black">{selectedEntity.name}</span></div>
                  <div className="grid grid-cols-4 gap-2 mb-3">
                    <div className="bg-slate-100 p-2.5 rounded-lg text-center"><div className="text-xs font-bold text-slate-600">رصيد افتتاحي</div><div className="text-base font-black">{totals.opening.toLocaleString("ar-EG")}</div></div>
                    <div className="bg-rose-50 p-2.5 rounded-lg text-center"><div className="text-xs font-bold text-rose-700">{isAll ? "إجمالي الفواتير" : "المدين"}</div><div className="text-base font-black">{totals.debit.toLocaleString("ar-EG")}</div></div>
                    <div className="bg-emerald-50 p-2.5 rounded-lg text-center"><div className="text-xs font-bold text-emerald-700">{isAll ? "إجمالي المدفوعات" : "الدائن"}</div><div className="text-base font-black">{totals.credit.toLocaleString("ar-EG")}</div></div>
                    <div className="bg-[#0284c7] p-2.5 rounded-lg text-center text-white"><div className="text-xs font-bold">الرصيد النهائي</div><div className="text-base font-black">{totals.balance.toLocaleString("ar-EG")}</div></div>
                  </div>
                </div>
                  ...
                  {isAll ? (
                    <table className="w-full text-right text-sm" dir="rtl">
                      <thead className="bg-gradient-to-r from-[#0284c7] to-[#0369a1] text-white text-xs print:bg-[#0284c7]">
                        <tr>
                          <th className="px-3 py-2.5 border-b border-[#0369a1] print:border-slate-400 font-bold w-10 text-center">#</th>
                          <th className="px-3 py-2.5 border-b border-[#0369a1] print:border-slate-400 font-bold">التاريخ</th>
                          <th className="px-3 py-2.5 border-b border-[#0369a1] print:border-slate-400 font-bold">اسم {getTypeLabel()}</th>
                          <th className="px-3 py-2.5 border-b border-[#0369a1] print:border-slate-400 font-bold text-center">حالة الحساب</th>
                          <th className="px-3 py-2.5 border-b border-[#0369a1] print:border-slate-400 font-bold bg-white/10">إجمالي المشتريات / الفواتير</th>
                          <th className="px-3 py-2.5 border-b border-[#0369a1] print:border-slate-400 font-bold bg-white/10">إجمالي المدفوعات</th>
                          <th className="px-3 py-2.5 border-b border-[#0369a1] print:border-slate-400 font-bold">صافي الرصيد المستحق</th>
                          <th className="px-3 py-2.5 border-b border-[#0369a1] print:border-slate-400 font-bold text-center print:hidden">التفاصيل</th>
                        </tr>
                      </thead>
                        ...
                        {summaryRows.length === 0 ? (
                          ...
                          <>
                            {summaryRows.map((row, idx) => (
                              <tr key={row.id} className="hover:bg-sky-50/40 dark:hover:bg-zinc-800/40 transition-colors">
                                <td className="px-3 py-2.5 text-xs text-slate-600 dark:text-slate-400 font-semibold text-center">{idx + 1}</td>
                                <td className="px-3 py-2.5 text-xs text-slate-600 dark:text-slate-400 font-semibold whitespace-nowrap">
                                  {new Date(row.createdAt).toISOString().split("T")[0]}
                                </td>
                                <td className="px-3 py-2.5 font-extrabold text-slate-900 dark:text-white text-sm">{row.name}</td>
                                <td className="px-3 py-2.5 text-center">
                                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-black ${row.statusColor}`}>
                                    {row.statusLabel}
                                  </span>
                                </td>
                                <td className="px-3 py-2.5 font-black text-slate-900 dark:text-slate-100 bg-rose-50/20 dark:bg-rose-950/10 text-sm">
                                  {row.totalInvoices.toLocaleString("ar-EG")}
                                </td>
                                <td className="px-3 py-2.5 font-black text-emerald-700 dark:text-emerald-400 bg-emerald-50/20 dark:bg-emerald-950/10 text-sm">
                                  {row.totalPayments.toLocaleString("ar-EG")}
                                </td>
                                <td className="px-3 py-2.5 font-black text-[#0284c7] dark:text-[#38bdf8] text-sm">
                                  {row.netBalance.toLocaleString("ar-EG")}
                                </td>
                                <td className="px-3 py-2.5 text-center print:hidden">
                                  <Link
                                    href={`/statement?type=${type}&id=${row.id}`}
                                    className="inline-flex items-center gap-1 text-xs font-bold text-[#0284c7] hover:underline"
                                  >
                                    كشف تفصيلي <ExternalLink className="w-3 h-3" />
                                  </Link>
                                </td>
                              </tr>
                            ))}
                            <tr className="bg-gradient-to-r from-[#0284c7] to-[#0369a1] text-white font-black print:bg-[#0284c7]">
                              <td colSpan={4} className="px-3 py-3 text-sm font-black">إجمالي كافة الحسابات النهائي</td>
                              <td className="px-3 py-3 text-white text-sm font-black">
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-zinc-800 print:divide-slate-300">
                        {movements.length === 0 ? (
                          ...
                          <>
                            ...
                            <tr className="bg-gradient-to-r from-[#0284c7] to-[#0369a1] text-white font-black print:bg-[#0284c7]">
                              <td colSpan={3} className="px-3 py-3 text-sm">الرصيد المستحق النهائي</td>
                              <td colSpan={2} className="px-3 py-3 text-white text-base font-black">
                                {(totals.balance || 0).toLocaleString("ar-EG")}
                              </td>
                            </tr>
